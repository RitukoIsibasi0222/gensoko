...
describe("PATCH /admin/users/:id/role", () => {
  ...
  it("role を service に渡して結果を返す", async () => {
  ...
  it("service の409を返す", async () => {
    vi.mocked(updateAdminUserRole).mockRejectedValue(
      new AdminServiceError(409, "自分自身には実行できません"),
    );

    const res = await app.request("/admin/users/admin-1/role", {
      method: "PATCH",
      headers: { Authorization: "Bearer admin-token", "Content-Type": "application/json" },
    ...
    expect(await res.json()).toEqual({ error: "自分自身には実行できません" });
  });
});
