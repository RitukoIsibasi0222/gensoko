...
- production projectはGit未接続のため、production environment scopeへPreview branch用の`--git-branch=main`を併用しないPDA-18を適用した。ただし後続runによりbranch scopeだけが原因ではなく、`vercel pull`自体が最小権限tokenへproduction環境変数とproject settingsの読み取りを要求することが判明した。

### 3回目production runのfail-closed記録（2026-08-07）

- PDA-18とrelease review follow-up PR #213を含むrelease PR #212のowner merge SHA `dbfb7d2021ec1a5be29bcc0ecb6fe1a54d200346`でrun [31149586816](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31149586816)を開始した。
- branch・SHA・backend/frontend quality・production DB target・migration current・provider credential・production API deploy・API healthは同じSHAで成功した。
- branch scopeを除去した`vercel pull`もproduction設定取得で固定error停止した。frontend candidate build/deploy、promote、read-only smokeは未実行で、safe evidence生成・uploadとcleanupは成功した。DB mutationはなく、APIは同SHAへ更新済み、公開frontendは直前版を維持した。
- production workflowは公開API URLを明示しているためprovider環境変数の取得は不要である。`vercel pull`とprovider側buildを使わず、`VERCEL_ENV=production`、exact SHA/ref metadata、明示した公開API URLでrepository buildを行い、Build Output contract検証後にCI環境変数で固定したproduction projectへprebuilt candidateをdeployする。修正のreview・develop/main昇格前にrunを再実行しない。

### 4回目production runのfail-closed記録（2026-08-07）

- PDA-19を含むrelease PR #215のowner merge SHA `0b6076687bad5ff42f11c76a16afe272a4c8f1ee`でrun [31151482177](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31151482177)を開始した。
- branch・SHA・backend/frontend quality・production DB target・migration current・provider credential・production API deploy・API health・frontend mutation直前のlive main・repository build・Vercel Build Output contractは同じSHAで成功した。
- `deploy --prebuilt --prod --skip-domain`は固定errorで停止し、candidate URLは得られず、promote・read-only smokeは未実行だった。safe evidence生成・uploadとcleanupは成功した。DB mutationはなく、APIは同SHAへ更新済み、公開frontendは直前版を維持した。
...
### 6回目production runのfail-closed記録（2026-08-07）

- PDA-21を含むrelease PR #219のowner merge SHA `7156bdf1bb8a9edd9f15b069d56084acbec87f72`でrun [31162801870](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31162801870)を開始した。
- branch・SHA・backend/frontend quality・production DB target・migration current・provider credential形式・Vercel team/project read-only preflight・production API deploy・API health・frontend mutation直前のlive main・repository build・Build Output contractは同じSHAで成功した。
...
- preflightは`teamId`付きAPIで同じproject IDを取得できたため、token・team・projectの不存在やread権限不足ではない。Vercel公式custom CI手順どおりproduction専用`VERCEL_ORG_ID`と`VERCEL_PROJECT_ID`環境変数だけをproject bindingに使い、CLI `deploy --project`と`list <project>`の重複selectorを除去する。外部Secret・resource・scopeは変更せず、review・develop/main昇格前にrunを再実行しない。

### 7回目production runのfail-closed記録（2026-08-07）

- PDA-22とPDA-23を含むrelease PR #221のowner merge SHA `4e7c16436c27468bd676c74394ba3bcb30820312`でrun [31167516423](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31167516423)を開始した。
- branch・SHA・backend/frontend quality・production DB target・migration current・provider credential・Vercel team/project read-only preflight・production API deploy・API health・frontend mutation直前のlive main・repository build・Build Output contractは同じSHAで成功した。
- candidate deployは固定category`project_not_found`で停止し、promote・read-only smokeは未実行だった。safe evidence生成・uploadとcleanupは成功し、DB mutationはなく、APIは同SHAへ更新済み、公開frontendは直前版を維持した。
