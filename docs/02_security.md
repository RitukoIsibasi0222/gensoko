...
### bcrypt入力長制限（未実装・フェーズ11で対応）

- bcryptjsは、UTF-8変換後の先頭72バイトだけをハッシュ対象とし、72バイトを超える部分を切り捨てる。
- 現在の共通 `strongPasswordSchema` は最小長と文字種を検証しているが、UTF-8の最大72バイトはまだ検証していない。
- 管理者作成CLIだけへ個別対応すると、通常登録・パスワード変更・パスワードリセットとの仕様が不一致になるため、フェーズ11の独立タスクとして認証全体を一括修正する。
- 独立タスクでは、通常登録、パスワード変更、パスワードリセット、管理者作成CLI、フロントエンド表示、既存ユーザーとの互換性を確認する。
- 境界値テストはASCIIだけでなく、1文字が複数バイトになる日本語などを含め、UTF-8で72バイトと73バイトのケースを追加する。
- 対応完了までは、運用で使用するパスワードをUTF-8で72バイト以内にする。
