...
### 11. DB構造変更時の確認（必須）
- [ ] `backend/prisma/schema.prisma` または `backend/prisma/migrations/` を変更した場合、マイグレーション適用を確認したか
- [ ] DB構造変更がある場合、**Playwrightでフロント動作確認**を実施したか
- [ ] Playwrightで、変更影響のある画面（最低1導線以上）を確認したか
- [ ] APIレスポンスだけでなく、画面上の成功/失敗表示が仕様どおりか確認したか

### 12. 仕様変更時のドキュメント更新（必須）
- [ ] API仕様変更がある場合、`docs/04_api.md` を更新したか
- [ ] タスク進捗・完了条件に変更がある場合、`docs/05_progress.md` を更新したか
- [ ] 実装計画に変更がある場合、対象の `docs/plans/{機能名}/plan.md` を更新したか
- [ ] ユーザー向け挙動（エラーメッセージ・遷移先・入力条件）が変わる場合、関連ドキュメントを更新したか
