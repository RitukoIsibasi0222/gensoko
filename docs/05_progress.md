...
- [-] staging frontend/API配備基盤（Workers専用entrypoint・Prisma/mail runtime境界・Wrangler・Vercel Preview・T34実機確認） — 計画書: [`docs/plans/staging-app-deployment/plan.md`](plans/staging-app-deployment/plan.md) — SD1〜SD8完了。SQLite-backed Durable ObjectとMailSender/Workers fetch mail adapterはlocal test・厳格review改善でGreen。SD9以降と外部配備は未実施
