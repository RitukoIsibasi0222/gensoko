...
### フロントエンド共通基盤
- [ ] SvelteKit ルーティング・共通レイアウト（ヘッダー/フッター/ナビ）
- [ ] 認証 Store（Svelte ストア・Bearer トークン管理・リフレッシュ自動実行）
- [ ] API クライアント基盤（`lib/api/client.ts`・共通エラーハンドリング）
- [ ] トースト通知コンポーネント（成功/エラー表示）

### アカウント系画面
- [ ] ログイン画面 `/login`（フォーム・エラー表示・「パスワードを忘れた」リンク）
- [ ] ユーザー登録画面 `/register`（入力・確認メール送信完了メッセージ）
- [ ] メール認証完了ページ `/verify-email`（トークン自動送信・成功/失敗メッセージ）
...
### 画面
- [ ] トップページ `/`（アプリ概要・ゲーム開始CTA・ランキングプレビュー）
- [ ] 元素一覧ページ `/elements`（118枚カードグリッド・分類色分け）
- [ ] 元素詳細モーダルコンポーネント（カードクリックで開く）
- [ ] 検索・フィルターUI（キーワード・分類・周期）
