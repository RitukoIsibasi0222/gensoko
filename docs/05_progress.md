...
- [-] develop merge後に固定staging frontendを自動更新する — Issue [#173](https://github.com/RitukoIsibasi0222/gensoko/issues/173) / PR: [#192](https://github.com/RitukoIsibasi0222/gensoko/pull/192) / 計画書: [`staging-frontend-auto-deploy`](plans/staging-frontend-auto-deploy/plan.md) — PR #192はmerge済み。承認後にstaging専用Vercel Secret 3件とIgnored Build Stepを反映したが、run 31072094165の再実行でteam IDをslug用`--scope`へ渡す不具合が判明した。aliasは未変更のまま安全に失敗し、project限定tokenを維持したCLI scope回帰修正を実装中
