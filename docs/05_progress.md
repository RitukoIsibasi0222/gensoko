...
- [ ] M2: 同じrelease候補SHAで、stagingの登録〜退会、通常password verifier DO、auth 429、基本keyboard/320px、cleanupを1回確認する — 計画書: [`docs/plans/m2-staging-release-candidate-campaign/plan.md`](plans/m2-staging-release-candidate-campaign/plan.md) — M1が同じSHAでPath Aに確定するまでstaging deploy/requestへ進まず、Path B/unknownなら通常gateへ戻る
