...
- [-] main merge後に承認付きでAPI → health → frontend → smokeを自動化する — Issue [#174](https://github.com/RitukoIsibasi0222/gensoko/issues/174) / 計画書: [`production-auto-deploy`](plans/production-auto-deploy/plan.md) / 計画PR: [#202](https://github.com/RitukoIsibasi0222/gensoko/pull/202) / 実装PR: [#203](https://github.com/RitukoIsibasi0222/gensoko/pull/203) / latest follow-up PR: [#228](https://github.com/RitukoIsibasi0222/gensoko/pull/228) / latest release PR: [#229](https://github.com/RitukoIsibasi0222/gensoko/pull/229) — main SHA `86d5d2625ed0158dfd0b2cf42f2bbe1a8246fc6c`のrun [31261605275](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31261605275)はVercel preflight、production API health、Build Output contractまで成功し、CLI candidate deployで安全停止した。同SHAのDashboard手動Production deploymentと公開画面確認は成功済み。PDA-27〜PDA-29でGit IntegrationのSTAGED候補待機とREST promoteへTDD移行し、develop staging確認・外部設定・PDA-16実run検証を残す
  - [x] PDA-21 Vercel credential preflightをTDD実装 — release PR [#217](https://github.com/RitukoIsibasi0222/gensoko/pull/217)のmain SHAでrun [31154972851](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31154972851)を5回安全停止した。各attemptはAPI healthまで成功し、candidate deployで`project_not_found`、promote・smoke未実行、safe evidence・cleanup成功、DB変更なし、旧frontend維持。token・team・projectの参照可否をresponse body非出力・HTTP status allowlistでAPI deploy前に段階判定するfollow-up実装と全品質gateを完了した。PDA-16の実production run検証は未完了
  - [x] PDA-22 Vercel CI project bindingをTDD修正 — PR #218とrelease PR #219のmerge後、main SHA `7156bdf1bb8a9edd9f15b069d56084acbec87f72`のrun [31162801870](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31162801870)はVercel read-only preflight、API deploy、API healthまで成功し、CLI candidate deployだけが`project_not_found`で安全停止した。promote・smoke未実行、DB変更なし、旧frontend維持。production専用org/project環境IDへbindingを一本化するfollow-up実装と対象検証を完了し、PDA-16は未完了を維持する
  - [x] PDA-23 candidate project境界とworkflow contractをTDD補強 — release PR #221のCopilot review 2件へ対応し、`vercel list`結果の`projectId`がproduction専用`VERCEL_PROJECT_ID`と一致する候補だけを許可するfail-closed検証を追加した。contract testはCLIコマンド全体の完全一致をやめ、必須・禁止引数を個別検証する形へ整理した。外部設定・deployment・DBは変更していない
  - [x] PDA-24 Vercel CLI owner lookupをTDD修正 — PR #221 merge SHA `4e7c16436c27468bd676c74394ba3bcb30820312`のrun [31167516423](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31167516423)はVercel preflight、API deploy、API health、repository buildまで成功し、candidate deployだけが`project_not_found`で安全停止した。CLI `56.3.2`の旧team owner lookup用cacheを既存Team IDの明示scopeで事前解決する最小修正をRed→Greenで実装し、全local品質gateを完了した。develop向けPR [#222](https://github.com/RitukoIsibasi0222/gensoko/pull/222)は全check成功、Copilot指摘なしでowner merge待ち。Secret・権限・provider設定は変更していない
  - [x] PDA-25 Vercel CLI Team slug scopeをTDD修正 — run [31258867516](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31258867516)と[31259360381](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31259360381)はteam preflightで安全停止し、All Projects tokenへ更新後のrun [31259602415](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31259602415)はAPI health後のcandidate deployだけが`project_not_found`となった。Team IDはCI bindingに維持し、read-only preflightで取得・検証したslugだけを一時参照経由でdeploy/list/promoteの`--scope`へ渡す修正をRed→Greenで実装し、全local品質gateを完了した。raw provider response・ID・slugは出力しない
  ...
  - [ ] PDA-30 develop staging確認・外部設定・production実run検証 — develop品質checkと固定staging画面を確認後、production Auto-assign OFF → Git接続 → main-only buildの順で設定し、release PRとsame SHAのAPI → health → STAGED candidate → promote → smokeを確認する
...
- [-] main承認付き通常release自動化 — Issue [#174](https://github.com/RitukoIsibasi0222/gensoko/issues/174) / 計画書: [`production-auto-deploy`](plans/production-auto-deploy/plan.md) / 実装PR: [#203](https://github.com/RitukoIsibasi0222/gensoko/pull/203) / latest follow-up PR: [#226](https://github.com/RitukoIsibasi0222/gensoko/pull/226) / latest release PR: [#227](https://github.com/RitukoIsibasi0222/gensoko/pull/227) — run [31260704440](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31260704440)はTeam slug preflightとAPI health後、candidate deployで安全停止した。PDA-26の現行CLI・explicit project修正をreview・develop/main昇格後にPDA-16で検証する
  - [x] PDA-21 repository follow-up — PR #216/#217 merge後のrun [31154972851](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31154972851)は5 attemptともAPI healthまで成功しcandidate deployで安全停止した。Vercel team/project preflightをTDD実装済み。feature PRとmain昇格後にPDA-16を再開する
  - [x] PDA-22 repository follow-up — PR #218/#219 merge後のrun [31162801870](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31162801870)でVercel preflightとAPI healthは成功し、重複project selectorを通るcandidate deployだけが安全停止した。公式custom CI方式に合わせて既存org/project環境IDへbindingを一本化し、feature PRとmain昇格後にPDA-16を再開する
  - [x] PDA-23 release review follow-up — PR #221のCopilot reviewに従い、candidate metadataのproduction project ID完全一致と、CLI必須・禁止引数単位の保守可能なcontract testを追加した。PR #221 merge後にPDA-16を再開する
  - [x] PDA-24 repository follow-up — run [31167516423](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31167516423)でAPI health後のcandidate deployだけが`project_not_found`となった。Vercel CLI owner lookup用team cacheを既存Team IDの明示scopeで事前解決する最小修正をTDD実装し、全local品質gateを完了した。develop向けPR [#222](https://github.com/RitukoIsibasi0222/gensoko/pull/222)は全check成功、Copilot指摘なしでowner merge待ち。外部設定変更と同条件rerunは行っていない
  - [x] PDA-25 repository follow-up — run [31258867516](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31258867516)と[31259360381](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31259360381)はteam preflightで安全停止し、All Projects tokenへ更新後のrun [31259602415](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31259602415)はAPI health後のcandidate deployだけが`project_not_found`となった。検証済みTeam slugを一時参照へ保存し、deploy/list/promoteの`--scope`へ渡すTDD修正と全local品質gateを完了した
  - [x] PDA-26 repository follow-up — run [31260704440](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31260704440)でcandidate deployだけが`project_not_found`となった。Vercel CLI `58.9.0`とpreflight済みproduction project IDの明示selectorをTDD実装し、全local品質gateを完了した
- [ ] 管理画面Playwright網羅
