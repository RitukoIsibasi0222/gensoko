...
- [-] M6: productionの単一synthetic Userで登録、転送メール受信、メール認証、login、reload後の認証維持まで成功した。Element APIは通信成功の空配列で、production DBに正本118元素が未投入であることをread-only確認したため、承認付きseed成功までgame以降を停止する。本人退会、User所有row cleanup、release記録、公開後引継ぎは未完了で、AuditLogは365日保持方針に従う
  - [-] Production元素seed手順: 初回失敗後に有限120秒timeout・10秒maxWait・workflow 3分上限を追加してPR #177/#178をmergeし、別承認で再実行した。2回目もmain・入力・DB target・migration currentは成功したがseed stepで即時失敗し、独立verify前に安全停止したため、timeoutを根因とする断定を撤回した。ローカルPostgreSQLでは既存正本118件と新規一時空DBの両経路でseed・独立verifyが成功した。production固有の既存状態不一致とDB transaction失敗をraw errorなしの固定カテゴリで区別する診断をTDD実装中で、review・develop/main再昇格・別承認まで再実行しない — 計画書: [`production-elements-seed`](plans/production-elements-seed/plan.md)
