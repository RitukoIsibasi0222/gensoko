...
- [-] 定期バッチ運用再設計（GameQuestionSet cleanup日次化・scheduled専用production境界・kill switch・滞留run解消） — repository実装BO1〜BO11完了 / PR: #166。source integrity gate、`production-batch`、旧run整理、kill switch有効化、初回run、14日baselineは未実施 — 計画書: [`batch-operations-redesign`](plans/batch-operations-redesign/plan.md)
