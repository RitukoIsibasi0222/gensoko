...
- [-] 退会時の個人情報・学習データ完全削除方針（本人退会・管理者強制退会・既存soft-deleted userの移行） — 計画書: [docs/plans/account-data-complete-deletion/plan.md](plans/account-data-complete-deletion/plan.md) — 監査ログ保持とは分離した本番公開前ブロッカー（実装中・本番公開gate未完了）
  ...
  - [-] T34/T35安全preflight: synthetic browser回帰、fixture完全一致preflight、staging synthetic本人退会・ゲーム・password resetを確認済み。T34初回runのVercel SSO redirectはorigin限定automation bypassで補正した。PR #120 merge後のrun 29788242095はprepare・両cleanup・flag `false`復旧に成功したが、login response待ちでtimeoutした。liveのclient validationとdummy login 401を確認し、hydration前submitが最有力と推定してsynthetic credential前のreadiness probeをTDD追加した。修正merge・再承認後のAdmin login・強制退会・旧credential 401と、T35 cleanup dry-run/execute/再実行は未実施
...
- [-] staging frontend/API配備基盤（Workers専用entrypoint・Prisma/mail runtime境界・Wrangler・Vercel Preview・T34実機確認） — 計画書: [`docs/plans/staging-app-deployment/plan.md`](plans/staging-app-deployment/plan.md) / PR: #117 — SD1〜SD13・SD15完了。Vercel `develop` Preview、Cloudflare staging Worker/DO/Hyperdrive/secret、Supabase migration、health/CORS/OPTIONS、元素118件、synthetic登録・認証・ゲーム・password reset・本人退会を確認済み。production resource・deploy・DB操作は未実施
  - [-] SD16実環境検証: 完全一致fixtureとmanual Playwright workflowを実装済み。run 29788242095はprepare成功後、login response待ちでtimeoutし、Admin強制退会・旧credential 401には未到達。main cleanup 2件、recovery cleanup対象0件、fixture残存なし、flag `false`復旧を確認した。liveのclient validationとdummy login 401からhydration前submitを最有力と推定し、synthetic credential前のreadiness probeをTDD追加した。修正merge・再preflight・明示承認後の再実行を残す。production・migration・実メール・再配備は未実行
