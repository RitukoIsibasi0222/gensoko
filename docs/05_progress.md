...
- [-] staging frontend/API配備基盤（Workers専用entrypoint・Prisma/mail runtime境界・Wrangler・Vercel Preview・T34実機確認） — 計画書: [`docs/plans/staging-app-deployment/plan.md`](plans/staging-app-deployment/plan.md) — SD1〜SD12が完了し、adapter-vercel固定、Preview build契約、frontend品質gate、staging runbook同期まで実施済み。SD13以降のCloudflare/Vercel/Supabase・実DBを伴う外部操作は未実施
