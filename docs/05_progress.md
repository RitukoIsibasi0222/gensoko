...
- [-] M6: productionの単一synthetic Userで登録、転送メール受信、メール認証、login、reload後の認証維持まで成功した。Element APIは通信成功の空配列で、production DBに正本118元素が未投入であることをread-only確認したため、承認付きseed成功までgame以降を停止する。固定カテゴリ診断をmainへ昇格した後の別承認付き第三回runもtarget・migration成功後のseed stepで汎用fallbackへ安全停止し、失敗後も元素一覧は空のままだった。CLI初期化・entrypoint起動前後を固定文言で区別する追加修正をreview・main昇格するまで再実行しない。本人退会、User所有row cleanup、release記録、公開後引継ぎは未完了で、AuditLogは365日保持方針に従う
  - [-] Production元素seed手順: 初回失敗後に有限120秒timeout・10秒maxWait・workflow 3分上限を追加してPR #177/#178をmergeし、別承認で再実行した。2回目もmain・入力・DB target・migration currentは成功したがseed stepで即時失敗し、独立verify前に安全停止したため、timeoutを根因とする断定を撤回した。ローカルPostgreSQLでは既存正本118件と新規一時空DBの両経路でseed・独立verifyが成功した。transaction内固定4分類をmainへ昇格した後の別承認付き3回目は、その分類へ到達せず汎用fallbackで安全停止した。CLI初期化・entrypoint起動前後をraw errorなしの固定カテゴリで区別する追加診断をTDD実装し、review・develop/main再昇格・さらに別の明示承認まで再実行しない — 計画書: [`production-elements-seed`](plans/production-elements-seed/plan.md)
