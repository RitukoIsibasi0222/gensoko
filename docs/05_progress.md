...
- [-] 退会時の個人情報・学習データ完全削除方針（本人退会・管理者強制退会・既存soft-deleted userの移行） — 計画書: [docs/plans/account-data-complete-deletion/plan.md](plans/account-data-complete-deletion/plan.md) — 監査ログ保持とは分離した本番公開前ブロッカー（実装中・本番公開gate未完了）
  ...
  - [-] T34/T35安全preflight: synthetic browser回帰、fixture完全一致preflight、staging synthetic本人退会・ゲーム・password resetを確認済み。Admin/User専用fixture・manual Playwright workflowを実装した。T34初回runはVercel SSO redirectで失敗し、origin限定automation bypassをTDD補正した。PR #120 merge後のrun 29788242095ではlogin画面へ到達し、prepare 2件、main cleanup 2件、recovery cleanup対象0件、flag `false`復旧を確認したが、固定Worker APIのlogin response待ちでtimeoutした。Admin login成功・強制退会・旧credential 401、T35 cleanup dry-run/execute/再実行は未実施
...
- [-] staging frontend/API配備基盤（Workers専用entrypoint・Prisma/mail runtime境界・Wrangler・Vercel Preview・T34実機確認） — 計画書: [`docs/plans/staging-app-deployment/plan.md`](plans/staging-app-deployment/plan.md) / PR: #117 — SD1〜SD13・SD15完了。Vercel `develop` Preview、Cloudflare staging Worker/DO/Hyperdrive/secret、Supabase migration、health/CORS/OPTIONS、元素118件、synthetic登録・認証・ゲーム・password reset・本人退会を確認済み。production resource・deploy・DB操作は未実施
  - [-] SD16実環境検証: 完全一致staging synthetic Admin/User fixture、`develop`限定manual workflow、固定Vercel/Worker URL向けPlaywright管理者強制退会E2EをTDD実装。初回runのVercel SSO redirectはorigin限定automation bypassで解消した。PR #120 merge後のrun 29788242095はprepare成功後、固定Worker APIのlogin response待ちでtimeoutし、Admin強制退会・旧credential 401には未到達。main cleanup 2件とrecovery cleanup対象0件でfixture残存なし、flag `false`復旧を確認した。原因診断と必要な補正後の再実行を残す。production・migration・実メール・再配備は未実行
