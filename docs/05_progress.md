...
- [-] APIレート制限の本番適用継続（Workers専用entrypoint・SQLite-backed Durable Object・WAF・staging/production実機確認） — 実装履歴: [`api-rate-limit-production`](plans/api-rate-limit-production/plan.md) / R7実環境gate正本: [`r7-rate-limit-environment-gates`](plans/r7-rate-limit-environment-gates/plan.md) — T13/T14のDO test・store adapterとstaging namespace/binding稼働は確認済み。PR #144 merge後の第三auth run 30056294929は許可request 1〜5回目を通過後、6回目のstatus 503を`EDGE_OR_UNCLASSIFIED_503`として安全停止した。fixture prepare 2件、main cleanup 2件、recovery追加削除0件、flag `false`復旧は成功し、再実行していない。503原因の切り分け、auth 11回目429成功証拠、制御された503契約、WAF、監視、production resource/preflight/smokeは未完了
...
- [-] R7: app rate limitの実環境gateを完了する — 計画書: [`r7-rate-limit-environment-gates`](plans/r7-rate-limit-environment-gates/plan.md) — R7-01の基準SHA・repository contract test 100件・Workers buildと、R7-03のstaging Worker/DO/Secret presence確認は完了。第三auth run 30056294929では新しいG5/G6承認、fixture prepare/main/recovery cleanup、flag `false`復旧が成功したためR7-04を完了した。許可request 1〜5回目は通過したが6回目の503で停止し、11回目429へ未到達のためR7-05は未完了。Cloudflare domain/zoneが0件でR7-02はblocked。503原因、監視、production分離、rollbackは未完了
  ...
  - [x] 503契約不一致詳細分類: [`r7-auth-503-contract-detail`](plans/r7-auth-503-contract-detail/plan.md) / PR: [#145](https://github.com/RitukoIsibasi0222/gensoko/pull/145) — raw値を出さずContent-Type、Retry-After、公開header名、JSON parse、単一error body、response accessの不一致箇所だけを固定分類した。Redはrunner 21件・CLI 1件失敗、Green 58件、関連66件、backend 1110件、Workers 15件、build・lint・format checkが成功。第三runへ遡及適用せず、第四run・delay追加・実環境変更は対象外
...
- [-] 認証系・ゲーム送信APIのアプリレベルrate limit — Hono/DO、staging binding、manual境界証拠workflowまで実装済み。auth初回run後に診断を補強し、第二runは許可request 5回目の503で安全にcleanup・flag復旧して停止した。503原因、実HTTP 429成功証拠、制御された503契約、production binding待ち
