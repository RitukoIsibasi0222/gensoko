...
- [-] main merge後に承認付きでAPI → health → frontend → smokeを自動化する — Issue [#174](https://github.com/RitukoIsibasi0222/gensoko/issues/174) / 計画書: [`production-auto-deploy`](plans/production-auto-deploy/plan.md) / 計画PR: [#202](https://github.com/RitukoIsibasi0222/gensoko/pull/202) / 実装PR: [#203](https://github.com/RitukoIsibasi0222/gensoko/pull/203) / bypass修正PR: [#205](https://github.com/RitukoIsibasi0222/gensoko/pull/205) / config path修正PR: [#208](https://github.com/RitukoIsibasi0222/gensoko/pull/208) — repository実装・review・develop merge、PDA-14外部設定、develop→main release PR #207のowner mergeまで完了。merge SHAのproduction runはbranch・SHA・quality・migration・credential gateを通過した後、production Worker一時configを`RUNNER_TEMP`へ生成したことで相対`main`/`$schema`の解決基準がbackend外へずれ、API deployでfail-closed停止した。API health以降、frontend、smoke、DB mutationは未実行。required reviewer・main限定policyを維持したままPDA-17のTDD修正・local品質gateを完了し、PR #208はmerge SHA `4ff6439c2c25215961e8e64a9cc63d8ad58fc9c4`としてdevelopへowner merge済み。release PR #209でmain昇格を進める
...
- [-] main承認付き通常release自動化 — Issue [#174](https://github.com/RitukoIsibasi0222/gensoko/issues/174) / 計画書: [`production-auto-deploy`](plans/production-auto-deploy/plan.md) / 計画PR: [#202](https://github.com/RitukoIsibasi0222/gensoko/pull/202) / 実装PR: [#203](https://github.com/RitukoIsibasi0222/gensoko/pull/203) / bypass修正PR: [#205](https://github.com/RitukoIsibasi0222/gensoko/pull/205) / config path修正PR: [#208](https://github.com/RitukoIsibasi0222/gensoko/pull/208) — release PR #207のowner mergeとproduction run開始まで完了。runはproduction Worker一時configの相対path解決位置がbackend外へずれたためAPI deployでfail-closed停止し、health・frontend・smoke・DB mutationは未実行。PDA-17のTDD修正・local品質gate・Copilot review（指摘なし）とPR #208のdevelop mergeは完了し、release PR #209のowner merge後にsame main SHA順序検証を再開する
- [ ] 管理画面Playwright網羅
