...
- [-] M6: productionの単一synthetic Userで登録、転送メール受信、メール認証、login、reload後の認証維持まで成功した。Element APIは通信成功の空配列で、production DBに正本118元素が未投入であることをread-only確認したため、承認付きseed成功までgame以降を停止する。CLI起動境界診断をmainへ昇格した後の別承認付き4回目もtarget・migration成功後のseed stepでCLI起動前カテゴリへ0秒で安全停止し、独立verifyはskip、失敗後も元素一覧は空のままだった。production workflowのmodule-load probeとNode+tsx直接entrypointをreview・main昇格するまで再実行しない。本人退会、User所有row cleanup、release記録、公開後引継ぎは未完了で、AuditLogは365日保持方針に従う
  - [-] Production元素seed手順: 初回失敗後に有限120秒timeout・10秒maxWait・workflow 3分上限を追加してPR #177/#178をmergeし、別承認で再実行した。2回目もmain・入力・DB target・migration currentは成功したがseed stepで即時失敗し、独立verify前に安全停止したため、timeoutを根因とする断定を撤回した。ローカルPostgreSQLでは既存正本118件と新規一時空DBの両経路でseed・独立verifyが成功した。transaction内固定4分類後の3回目は汎用fallback、CLI初期化・entrypoint分類後の4回目は起動前カテゴリへ安全停止した。DB target・migration・remote main sourceは正常で、失敗stepは0秒、元素一覧は空のままだった。productionだけNode+tsx直接entrypointと書込前module-load probeへ変更する修正をTDD実装し、review・develop/main再昇格・さらに別の明示承認まで再実行しない — 計画書: [`production-elements-seed`](plans/production-elements-seed/plan.md)
