...
- [-] main merge後に承認付きでAPI → health → frontend → smokeを自動化する — Issue [#174](https://github.com/RitukoIsibasi0222/gensoko/issues/174) / 計画書: [`production-auto-deploy`](plans/production-auto-deploy/plan.md) — production frontendの所有権をGitHub Actionsへ一本化し、same SHA、required reviewer、migration停止、safe evidence、staging/production分離を設計中
...
- [-] main承認付き通常release自動化 — Issue [#174](https://github.com/RitukoIsibasi0222/gensoko/issues/174) / 計画書: [`production-auto-deploy`](plans/production-auto-deploy/plan.md) — 計画PR作成中
- [ ] 管理画面Playwright網羅
