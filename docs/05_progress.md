...
- [-] main merge後に承認付きでAPI → health → frontend → smokeを自動化する — Issue [#174](https://github.com/RitukoIsibasi0222/gensoko/issues/174) / 計画書: [`production-auto-deploy`](plans/production-auto-deploy/plan.md) / 計画PR: [#202](https://github.com/RitukoIsibasi0222/gensoko/pull/202) — production frontendの所有権をGitHub Actionsへ一本化し、same SHA、required reviewer、migration停止、safe evidence、staging/production分離のrepository実装とlocal最終品質gateを完了。feature PR review、外部設定分離、develop→main release、production run検証は未実施
...
- [-] main承認付き通常release自動化 — Issue [#174](https://github.com/RitukoIsibasi0222/gensoko/issues/174) / 計画書: [`production-auto-deploy`](plans/production-auto-deploy/plan.md) / 計画PR: [#202](https://github.com/RitukoIsibasi0222/gensoko/pull/202) — repository実装中。外部production設定・実deployment・release mergeは直前承認へ分離する
- [ ] 管理画面Playwright網羅
