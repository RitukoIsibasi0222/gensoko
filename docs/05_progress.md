...
- [-] M6: productionの単一synthetic Userで登録、転送メール受信、メール認証、login、reload後の認証維持まで成功した。Element APIは通信成功の空配列で、production DBに正本118元素が未投入であることをread-only確認したため、承認付きseed成功までgame以降を停止する。Node+tsx直接entrypointをmainへ昇格した後の別承認付き5回目もtarget・migration成功後、書込前module-load probeで`module_load_failure`へ安全停止し、書込み用CLI本体とDB transactionには未到達、独立verifyはskip、元素一覧は0件のままだった。Prisma Client生成修正をreview・develop/mainへ再昇格し、さらに別の明示承認を得るまで再実行しない。本人退会、User所有row cleanup、release記録、公開後引継ぎは未完了で、AuditLogは365日保持方針に従う
  - [-] Production元素seed手順: 初回失敗後のtimeout修正、transaction内固定分類、CLI起動境界分類、Node+tsx直接entrypointを順にreview・main昇格したが、別承認付き5回目は書込前module-load probeで`module_load_failure`へ停止した。全5回で独立verifyはskip、元素一覧は0件のままであり、失敗workflowは再実行しない。productionへの直接CLI・SQL seedを禁止し、Prisma Client生成修正のreview・develop/main再昇格・さらに別の明示承認まで停止する — 計画書: [`production-elements-seed`](plans/production-elements-seed/plan.md)
    - [x] 第五回の`module_load_failure`修正: production接続なしのfresh `npm ci`直後にPrisma Client生成物不在とmodule import失敗を再現し、`prisma generate`後の同import成功を確認した。production workflowへseed時限定の生成stepを`npm ci`後・DB接続・module-load probe前に追加し、生成log非表示を契約testで固定した。対象12件・関連28件、backend全1306件・Workers 32件、build・Workers build・lint・format・Prisma validate・正本整形が成功した。review・develop/main昇格・別承認付き再実行は未実施
