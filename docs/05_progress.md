...
- [-] main承認付き通常release自動化 — Issue [#174](https://github.com/RitukoIsibasi0222/gensoko/issues/174) / 計画書: [`production-auto-deploy`](plans/production-auto-deploy/plan.md) / 実装PR: [#203](https://github.com/RitukoIsibasi0222/gensoko/pull/203) / latest follow-up PR: [#220](https://github.com/RitukoIsibasi0222/gensoko/pull/220) / latest release PR: [#221](https://github.com/RitukoIsibasi0222/gensoko/pull/221) — run [31167516423](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31167516423)はAPI health後、Vercel CLI owner lookupでcandidate deployを安全停止し、旧frontend・DBを維持した。PDA-24のrepository修正と全local品質gateを完了し、review・develop/main昇格後にPDA-16を再開する
  ...
  - [x] PDA-24 repository follow-up — run [31167516423](https://github.com/RitukoIsibasi0222/gensoko/actions/runs/31167516423)でAPI health後のcandidate deployだけが`project_not_found`となった。Vercel CLI owner lookup用team cacheを既存Team IDの明示scopeで事前解決する最小修正をTDD実装し、全local品質gateを完了した。外部設定変更と同条件rerunは行っていない
- [ ] 管理画面Playwright網羅
