...
- [-] staging frontend/API配備基盤（Workers専用entrypoint・Prisma/mail runtime境界・Wrangler・Vercel Preview・T34実機確認） — 計画書: [`docs/plans/staging-app-deployment/plan.md`](plans/staging-app-deployment/plan.md) — SD1〜SD9と厳格review改善が完了。公開500/503契約、production DO結合test、CI gate、生成binding型、bundle metadata検証を強化済み。SD10以降と外部配備は未実施
