...
- [-] main merge後に承認付きでAPI → health → frontend → smokeを自動化する — Issue [#174](https://github.com/RitukoIsibasi0222/gensoko/issues/174) / 計画書: [`production-auto-deploy`](plans/production-auto-deploy/plan.md) / 計画PR: [#202](https://github.com/RitukoIsibasi0222/gensoko/pull/202) / 実装PR: [#203](https://github.com/RitukoIsibasi0222/gensoko/pull/203) — repository実装はreview済みでdevelopへmerge済み。承認後にproduction専用Vercel project、Git非接続、project限定token、Cloudflare最小権限token、production Environment deploy Secret 5件を分離した。baselineはsame `main` SHA・production target・READY・project境界まで一致したが、Vercel Hobby Standard Protectionでcandidate content gateが認証画面となったためcustom domain移管前に停止。automation bypassのfail-closed対応、外部bypass設定、develop→main release、production run検証を継続する
...
- [-] main承認付き通常release自動化 — Issue [#174](https://github.com/RitukoIsibasi0222/gensoko/issues/174) / 計画書: [`production-auto-deploy`](plans/production-auto-deploy/plan.md) / 計画PR: [#202](https://github.com/RitukoIsibasi0222/gensoko/pull/202) / 実装PR: [#203](https://github.com/RitukoIsibasi0222/gensoko/pull/203) — repository実装・review・develop merge済み。外部設定はVercel automation bypassとcustom domain移管を残し、release mergeとproduction runは未実施
- [ ] 管理画面Playwright網羅
