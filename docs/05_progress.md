...
- [-] develop merge後に固定staging frontendを自動更新する — Issue [#173](https://github.com/RitukoIsibasi0222/gensoko/issues/173) / PR: [#192](https://github.com/RitukoIsibasi0222/gensoko/pull/192), [#193](https://github.com/RitukoIsibasi0222/gensoko/pull/193) / 計画書: [`staging-frontend-auto-deploy`](plans/staging-frontend-auto-deploy/plan.md) — PR #193のmerge SHA `ef97e98`でrun 31076459494のexact Preview探索とdevelop先端確認は成功したが、alias actionは更新前後の検証stepで失敗し固定aliasを旧bundleに維持した。project限定tokenで不要なProjects REST権限面を避け、pinned CLI JSONのproject name完全一致と秘密非表示の段階別診断へ修正中
