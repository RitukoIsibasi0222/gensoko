...
| ブラウザ確認 | OK: /weak が 404 ではなく仮ページ表示。スマホ幅でロゴ + メニューボタンのみ、開閉時 aria-expanded=true、閉状態 opacity=0/grid rows=0px、開状態 opacity=1/grid rows>0、開閉時のハンバーガー/バツ変形、閉状態 aria-hidden=true/inert 付与、認証初期化中のログイン後導線非表示条件を確認 |
