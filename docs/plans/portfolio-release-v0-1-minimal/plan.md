...
- [x] M6: Prisma Client生成修正のreview・develop/main昇格後、失敗runを再実行しない新規承認付きseedで原子的upsertと独立118件verifyが成功した。元素一覧、game、ranking、mypage、weak、通常password verifier DO、最小429とreset、password reset、本人退会を確認し、synthetic User、本人退会用一時GitHub設定、M6専用Email Routingルールをcleanupした。本人退会flagとscheduled batchは`false`、BO15は無効、active workflowは0件を維持した

2026-08-05の再開時点で、release PRのmain merge、確定main SHAのM3、M1R、M5のAPI・frontend deployと両health確認は完了している。M6の単一synthetic Userは登録、転送メール受信、メール認証、login、reload後の認証維持まで成功したが、Element APIが正常な空配列を返し、production DBに正本118元素が未投入であることをread-only確認したため停止した。承認付きseed手順の初回実行と、有限timeout修正をreview・main昇格した後の別承認付き再実行は、どちらもmain・入力・DB target・migration currentの検証成功後にseed stepで失敗し、独立verify前に安全停止した。2回目は120秒上限到達ではないためtimeoutを根因とする断定を撤回し、ローカルPostgreSQLの既存正本118件・新規一時空DBで同じCLIが成功することを確認した。transaction内4分類後の別承認付き3回目は汎用fallback、CLI起動境界分類後の別承認付き4回目は起動前カテゴリへ0秒で停止した。Node+tsx直接entrypoint後の別承認付き5回目は書込前module-load probeで`module_load_failure`へ停止し、書込み用CLI本体とDB transactionには未到達だった。全5回で独立verifyはskip、元素一覧は0件のままである。production接続・Secretなしのfresh `npm ci`直後にPrisma Client生成物不在とmodule import失敗、`npx prisma generate`後の生成物作成と同import成功を再現したため、[`production-elements-seed`](../production-elements-seed/plan.md)のseed時限定生成stepをreview・develop/mainへ再昇格し、さらに別承認付きworkflowが成功するまでgame以降へ進まない。synthetic UserとM6専用メールルーティングルールはcleanup工程まで維持し、BO15とscheduled batch kill switchは有効化しない。

2026-08-06にPrisma Client生成修正をreview・develop/mainへ再昇格し、失敗runを再実行せず、別の明示承認による新規runでseedと独立118件verifyが成功した。公開・認証済み元素一覧118件、game、ranking、mypage、weak、通常password verifier DO、最小429とreset、password resetを確認した。本人退会workflowのmain/recoveryは固定失敗として終了し再実行せず、再認証後の通常UIで対象identityを再確認して物理削除を完了した。削除成功表示、再設定メール非発行、保護画面拒否、ランキング消失を独立確認し、追加検証Userもlogin拒否・再設定メール非発行で不在を確認した。本人退会用一時Secret 2件・対象Variable・M6専用Email Routingルールを削除し、本人退会flagとscheduled batchは`false`、BO15は無効、active workflowは0件を維持した。AuditLogは365日保持方針に従い、M6を完了する。
...
9. production DBに正本118元素が未投入の場合だけ、review済みmain SHAの`Production Database Operations`へ追加した`seed-elements`を別承認で1回実行する。`npm ci`後かつDB接続・module-load probe前にseed時限定でPrisma Clientを生成し、生成logを表示しない。main、required reviewer、SHA、確認文字列、承認記録、production接続先、migration current、原子的upsert、別processの118件完全一致を値非表示確認する。直接CLI・SQL・staging workflowは使用しない。
