...
| `.github/workflows/staging-database.yml`                                                                                | 修正     | 通常migration適用を維持しつつ、T33計測モードで対象migration所要時間・synthetic write待ち・cleanup状態・最終statusを安全に記録            |
| `backend/src/jobs/stagingDatabaseWorkflow.test.ts`                                                                      | 修正     | 通常適用・T33計測の分岐、計測迂回防止、失敗後status確認、直列化・秘密非出力契約                                                          |
