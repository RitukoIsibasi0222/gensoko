...
| `frontend/src/lib/components/home/RankingPreviewSection.svelte` | 修正 | loading / error / empty / retry の表示状態を追加 |
...
| T5 | トップページで週間ランキング preview を取得する | `frontend/src/routes/(app)/+page.svelte` | 高 | AbortController と再試行を実装 |
| T6 | 品質チェックを実行する | `frontend` | 高 | `npm run check`, `npm run lint`, `npm run test:run` |
| T7 | 手動確認を実施する | `/` | 高 | loading, success, empty, error, `/ranking?period=weekly` 導線 |
...
- `AbortController` を使い、コンポーネント破棄時や再試行時に古い request が後勝ちしないようにする。
...
| A11Y | loading は `aria-live`、error は `role=alert` で伝わる |
